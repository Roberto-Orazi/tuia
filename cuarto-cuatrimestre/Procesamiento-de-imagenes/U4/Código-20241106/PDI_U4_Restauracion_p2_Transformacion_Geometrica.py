...
plt.subplot(221), imshow(
    img, title="Imagen Original", new_fig=False, colorbar=False, ticks=True
)
plt.subplot(222), imshow(
    img_rot_1, title="Imagen Rotada", new_fig=False, colorbar=False, ticks=True
)
plt.subplot(223), imshow(
    img_rot_2,
    title="Imagen Rotada - Tamaño duplicado",
    new_fig=False,
    colorbar=False,
    ticks=True,
)
...
plt.figure()
ax1 = plt.subplot(221)
imshow(img, title="Imagen Original", new_fig=False, colorbar=False, ticks=True)
...
img_rot_3a = cv2.warpAffine(
    img, M, (cols * k, rows * k), borderMode=cv2.BORDER_CONSTANT, borderValue=127
)
img_rot_3b = cv2.warpAffine(
    img, M, (cols * k, rows * k), borderMode=cv2.BORDER_REPLICATE
)
...
plt.figure()
ax1 = plt.subplot(221)
imshow(img_rot_3a, title="BORDER_CONSTANT", new_fig=False, colorbar=False)
plt.subplot(222, sharex=ax1, sharey=ax1), imshow(
    img_rot_3b, title="BORDER_REPLICATE", new_fig=False, colorbar=False
)
plt.subplot(223, sharex=ax1, sharey=ax1), imshow(
    img_rot_3c, title="BORDER_REFLECT", new_fig=False, colorbar=False
)
plt.subplot(224, sharex=ax1, sharey=ax1), imshow(
    img_rot_3d, title="BORDER_WRAP", new_fig=False, colorbar=False
)
plt.show(block=False)
...
# ==============================================================================
# En una transformación afín, todas las líneas paralelas de la imagen original seguirán siendo paralelas en la imagen de
# salida. Para encontrar la matriz de transformación, necesitamos tres puntos de la imagen de entrada y sus
# correspondientes ubicaciones en la imagen de salida. Entonces cv2.getAffineTransform creará una matriz de 2x3 que se
...
pts1 = np.float32(
    [[50, 50], [200, 50], [50, 200]]
)  # Defino 3 puntos en la imagen de entrada...
pts2 = np.float32(
    [[10, 100], [200, 50], [100, 250]]
)  # Defino sus correspondientes posiciones en la imagen de salida...
...
cv2.circle(
    img_warp, tuple(np.int32(pts2[0])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_warp, tuple(np.int32(pts2[1])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_warp, tuple(np.int32(pts2[2])), radius=5, color=(255, 0, 0), thickness=-1
)
...
pts2 = np.float32(
    [[0, 0], [2 * (cols - 1), 0], [0, 2 * (rows - 1)]]
)  # Escalo x2 en ambos ejes
M = cv2.getAffineTransform(pts1, pts2)  # Obtengo la matriz de la transformación afín.
...
M = np.array([[1.0, 0.0, 100], [0.0, 1.0, 200]])
img_warp = cv2.warpAffine(img, M, (cols * 2, rows * 2))
plt.figure()
ax1 = plt.subplot(121)
plt.imshow(img), plt.title("Input")
...
# --- Ejemplo: Escalado 2 -------------------------------------------------------
M = np.array([[2.0, 0.0, 0], [0.0, 2.0, 0]])
img_warp = cv2.warpAffine(img, M, (cols * 2, rows * 2))
plt.figure()
ax1 = plt.subplot(121)
plt.imshow(img), plt.title("Input")
...
M = np.array([[1.0, 0.8, 0.0], [0.0, 1.0, 0.0]])
M = np.array([[1.0, -0.8, 0.0], [0.0, 1.0, 0.0]])
M = np.array([[1.0, -0.8, 400], [0.0, 1.0, 0.0]])
img_warp = cv2.warpAffine(img, M, (cols * 2, rows * 2))
plt.figure()
ax1 = plt.subplot(121)
plt.imshow(img), plt.title("Input")
...
M = np.array([[1.0, 0.0, 0.0], [0.4, 1.0, 0.0]])
# M = np.array([[1., 0., 0.],[-0.4, 1., 0.]]) M = np.array([[1., 0., 0.],[-0.4, 1., 300]])
img_warp = cv2.warpAffine(img, M, (cols * 2, rows * 2))
plt.figure()
ax1 = plt.subplot(121)
plt.imshow(img), plt.title("Input")
...
pts_src = np.array(
    [[333, 82], [771, 166], [665, 775], [183, 669]]
)  # sup-izq | sup-der | inf-der | inf-izq
img_points = img.copy()
cv2.circle(
    img_points, tuple(np.int32(pts_src[0])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_points, tuple(np.int32(pts_src[1])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_points, tuple(np.int32(pts_src[2])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_points, tuple(np.int32(pts_src[3])), radius=5, color=(255, 0, 0), thickness=-1
)
...
im_dst.shape
cv2.circle(
    im_dst, tuple(np.int32(pts_dst[0])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    im_dst, tuple(np.int32(pts_dst[1])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    im_dst, tuple(np.int32(pts_dst[2])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    im_dst, tuple(np.int32(pts_dst[3])), radius=5, color=(255, 0, 0), thickness=-1
)
...
plt.figure()
ax1 = plt.subplot(122)
plt.imshow(im_dst), plt.title("Imagen Transformada")
plt.subplot(121, sharex=ax1, sharey=ax1), plt.imshow(img_points), plt.title(
    "Imagen Original"
)
plt.show(block=False)
...
cv2.circle(
    im_dst2, tuple(np.int32(pts_dst2[0])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    im_dst2, tuple(np.int32(pts_dst2[1])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    im_dst2, tuple(np.int32(pts_dst2[2])), radius=5, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    im_dst2, tuple(np.int32(pts_dst2[3])), radius=5, color=(255, 0, 0), thickness=-1
)
...
# Agrego los puntos
cv2.circle(
    img_warp, tuple(np.int32(pts2[0])), radius=10, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_warp, tuple(np.int32(pts2[1])), radius=10, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_warp, tuple(np.int32(pts2[2])), radius=10, color=(255, 0, 0), thickness=-1
)
# Muestro
plt.figure()
...
# Obtengo puntos de los extremos del libro
pts_src = np.array(
    [[433, 438], [542, 438], [910, 608], [80, 608]]
)  # sup-izq | sup-der | inf-der | inf-izq
img_points = img.copy()
cv2.circle(
    img_points, tuple(np.int32(pts_src[0])), radius=5, color=(255, 0, 0), thickness=-1
...
# Agrego los puntos
cv2.circle(
    img_warp2, tuple(np.int32(pts3[0])), radius=10, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_warp2, tuple(np.int32(pts3[1])), radius=10, color=(255, 0, 0), thickness=-1
)
cv2.circle(
    img_warp2, tuple(np.int32(pts3[2])), radius=10, color=(255, 0, 0), thickness=-1
)
# Muestro
plt.figure()
plt.subplot(121), plt.imshow(img_points), plt.title("Input")
