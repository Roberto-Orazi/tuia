...
N = 10000  # Probar 1000 - 10000 - ...
RGBlist = np.random.randint(0, 255, (N, 3))
colors = [
    (RGBlist[ii, 0] / 255.0, RGBlist[ii, 1] / 255.0, RGBlist[ii, 2] / 255.0)
    for ii in range(N)
]
...
colors = [
    (
        img_unique_rgbs[ii, 0] / 255.0,
        img_unique_rgbs[ii, 1] / 255.0,
        img_unique_rgbs[ii, 2] / 255.0,
    )
    for ii in range(img_unique_rgbs.shape[0])
]
...
ax = fig.add_subplot(111, projection="3d")
ax.scatter(
    img_unique_rgbs[:, 0], img_unique_rgbs[:, 1], img_unique_rgbs[:, 2], c=colors
)
# ax.grid(False)
ax.set_xlabel("R")
ax.set_ylabel("G")
...
colours, counts = np.unique(
    img_all_rgb_codes, axis=0, return_counts=1
