...
# ======================================================================================= UNIDAD 5: Color PARTE 1:
# Imágenes RGB e Imágenes indexadas
# =======================================================================================
...
img.dtype
imshow(
    img, title="Imagen Original"
)  # Acá puede verse que OpenCV, por default, carga las imágenes color en formato BGR, no RGB.

# --- Acomodamos canales ---------------------------------------------------------------- Por default, openCV  carga las
# imágenes color en formato BGR, no RGB. Por lo tanto, re-acomodamos los planos: pasamos de BGR a RGB.
...
plt.figure(), plt.imshow(R, cmap="gray"), plt.title(
    "Canal R"
), plt.colorbar(), plt.show(
    block=False
)  # Analizamos solo el plano R.

plt.figure()  # Analizamos todos los planos juntos.
ax1 = plt.subplot(221)
imshow(img_RGB, title="Imagen RGB", new_fig=False)
...
# --- Modifico un canal --------------------------------------------------------------- Anulamos el canal R, es decir,
# le asignamos valor 0 en todos los pixels de la imagen. img2 = img_RGB    # No! así crea una referencia: si se modifica
...
R2 = R2.astype(
    np.uint8
)  # Paso a uint8, ya que en la multiplicación anterior, automáticamente se hizo un casting a float
...
plt.subplot(222, sharex=ax1, sharey=ax1), imshow(
    img2, title="Canal R anulado", new_fig=False
)
plt.subplot(223, sharex=ax1, sharey=ax1), imshow(
    img3, title="Canal R escalado", new_fig=False
)
plt.show(block=False)

# --- Paleta de Colores --------------------------------------------------------------- Obtengamos los colores únicos de
# la imagen, es decir, su paleta de colores.
img_pixels = img_RGB.reshape(
    -1, 3
)  # Re-ordeno los pixels, de manera que cada pixel ocupe una fila.
colours, counts = np.unique(
    img_pixels, axis=0, return_counts=True
)  # Obtengo los valores únicos de todos los pixels y su frecuencia de aparición.
# colours = np.unique(img_pixels, axis=0)                           #   Así obtendría solo los valores únicos, no su
...
colours = colours[
    idx
]  # El ultimo elemento de counts posee el color con mayor frecuencia de aparición.
...
N1p = (
    100 * np.sum(counts == 1) / N_colours
)  # Es decir, un 80% de los colores de la paleta aparecen solo 1 vez.
print(
    f"{N1} colores de la paleta aparecen solo 1 vez en la imagen ({N1p:5.2f}% del total de colores de la paleta)."
)
...
img = cv2.imread(
    "peppers.png"
)  # N_colours= 99.059  --> Nbytes_img_idx/Nbytes_img: uint32 = 1.83
# img = cv2.imread('home.jpg')            # N_colours= 51.711  --> Nbytes_img_idx/Nbytes_img: uint32 = 1.59 | uint16 =
# 0.93
# img = cv2.imread('flowers.tif')         # N_colours= 120.260 --> Nbytes_img_idx/Nbytes_img: uint32 = 1.667  # Cuidado!
# puede demorar mucho en correr...
img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
print(img.shape)
imshow(img, title="Imagen Original")

# Obtengamos su paleta de colores
img_pixels = img.reshape(
    -1, 3
)  # Re-ordeno los pixels, de manera que cada pixel ocupe una fila.
colours, counts = np.unique(
    img_pixels, axis=0, return_counts=True
...
for ii in range(N_colours):
    ...
    img_idx[mask] = ii
    # --- Version compacta ------------------------------------------------------------------- img_idx[(img[:,:,0] ==
    # colours[ii][0]) & (img[:,:,1] == colours[ii][1]) & (img[:,:,2] == colours[ii][2])] = ii --- Otra version
    # ------------------------------------------------------------------------- mask = cv2.inRange(img, colours[ii],
    # colours[ii])  # https://docs.opencv.org/3.4/d2/de8/group__core__array.html#ga48af0ab51e36436c5d04340e036ce981
    # img_idx[mask>0] = ii
...
ax1 = plt.subplot(221)
plt.xticks([]), plt.yticks([]), plt.imshow(img), plt.title(
    "Imagen Original"
), plt.colorbar()
plt.subplot(222, sharex=ax1, sharey=ax1), plt.imshow(img_idx, cmap="gray"), plt.title(
    "Imagen Indexada - Indices"
), plt.colorbar()
plt.subplot(223, sharex=ax1, sharey=ax1), plt.imshow(img_idx, cmap="jet"), plt.title(
    "Imagen Indexada - jet"
), plt.colorbar()
plt.subplot(224, sharex=ax1, sharey=ax1), plt.imshow(img_idx, cmap="hot"), plt.title(
    "Imagen Indexada - hot"
), plt.colorbar()
plt.show(block=False)
...
img_idx_topValues_mask = cv2.inRange(
    img_idx, (N_colours - 1) * (100 - P) / 100, (N_colours - 1)
)  # Filtro el top P% de colores, en los demás pixels asigna 0
img_idx_topValues = cv2.bitwise_and(
    img, img, mask=img_idx_topValues_mask
)  # Hacemos un AND de la máscara anterior con la imagen original
# img_idx_topValues = img.copy()                                # Lo mismo que antes...
# img_idx_topValues[img_idx_topValues_mask<255, :] = 0          # pero de manera "manual".
porc = (
    100 * np.sum(img_idx_topValues_mask > 0) / np.prod(img_idx_topValues_mask.shape)
)  # Obtengo el porcentaje del total de pixels que cumplen esta condición

plt.figure()
ax1 = plt.subplot(221)
imshow(img, title="Imagen Original", new_fig=False)
plt.subplot(222, sharex=ax1, sharey=ax1), imshow(
    img_idx_topValues_mask,
    title=f"Imagen Indexada - {P:5.2f}% top values - mask",
    new_fig=False,
)
plt.subplot(223, sharex=ax1, sharey=ax1), imshow(
    img_idx_topValues,
    title=f"Imagen Indexada - top values (%{porc:5.2f})",
    new_fig=False,
)
plt.show(block=False)

# Veamos si logramos ahorrar espacio (~comprimir) al pasar de imagen RGB a imagen indexada Conversion
