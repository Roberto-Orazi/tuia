...
Z = img.reshape(
    (-1, 3)
)  # Genero una matriz de Nx3 con todos los valores de los pixels de la imagen (N = ancho x alto)
...
plt.xticks([]), plt.yticks([]), plt.imshow(img), plt.title(
    f"Imagen Original. Ncolors = {Ncolors}"
)
plt.subplot(122, sharex=ax1, sharey=ax1), plt.imshow(imgQ), plt.title(
    f"Imagen con colores cuantizados --> K = {K}"
)
plt.show(block=False)

# --- Gráfico de torta ------------------------------------------------------------------------
colours, counts = np.unique(
    imgQ.reshape(-1, 3), axis=0, return_counts=1
...
# --- Muestro mascaras -------------------------------------------------------- !!! Esta hecho para kmeans con k=8 !!!
plt.figure()
ax1 = plt.subplot(331)
...
for ii in range(8):
    ...
    plt.title(
        f"Color ({col_sel[0]},{col_sel[1]},{col_sel[2]}): {counts_pct[ii]:5.2f}% ({counts[ii]}) pixels"
    )
plt.show(block=False)
...
for ii in range(8):
    ...
    # --- Opcion binario ----------------- mask = np.uint8(mask_R & mask_B & mask_B)*255 plt.imshow(mask, cmap='gray')
    # --- Opcion color --------------------
