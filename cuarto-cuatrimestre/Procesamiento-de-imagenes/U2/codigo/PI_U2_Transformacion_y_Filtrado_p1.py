...
def imadjust(x, vin=None, vout=[0, 255], gamma=1):
    # x      : Imagen de entrada en escalas de grises (2D), formato uint8. vin    : Límites de los valores de intensidad
    # de la imagen de entrada vout   : Límites de los valores de intensidad de la imagen de salida y      : Imagen de
    # salida
    if vin == None:
        vin = [x.min(), x.max()]
    ...
    if x.dtype == np.uint8:
        y = np.uint8(
            np.clip(np.round(y), 0, 255)
        )  # Numpy underflows/overflows para valores fuera de rango, se debe utilizar clip.
    return y
...
plt.subplot(222, sharex=ax1, sharey=ax1), plt.imshow(
    img_neg, cmap="gray", vmin=0, vmax=255
), plt.title("Negativo")
plt.subplot(223, sharex=ax1, sharey=ax1), plt.imshow(
    img_adj, cmap="gray", vmin=0, vmax=255
), plt.title("imadjust(img,vin=(128,192))")
plt.subplot(224, sharex=ax1, sharey=ax1), plt.imshow(
    img_adj2, cmap="gray", vmin=0, vmax=255
), plt.title("imadjust(img, gamma=2)")
plt.show()
...
img2 = 1 / (
    1 + (np.mean(img) / (img + np.finfo(float).eps)) ** 0.9
)  # Al sumar un float y al calcular la media, el tipo de dato se transforma en float
