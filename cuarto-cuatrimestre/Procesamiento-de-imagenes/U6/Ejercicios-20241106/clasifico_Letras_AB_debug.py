...
for i in range(1, num_labels):
    ...
    # # *** Debug **************************
    # obj_color = cv2.merge((obj,obj,obj))*255
    # cv2.drawContours(obj_color, contours, -1, (255,0,0), 1)
    # st = stats[i,:]
    # cv2.rectangle(obj_color, (st[0], st[1]), (st[0]+st[2], st[1]+st[3]), color=(0,255,0), thickness=1)
    # plt.figure(), plt.imshow(obj_color), plt.title(f"label {i}"), plt.show()
    # # ************************************
...
for i in range(1, num_labels):
    st = stats[i,:]
    obj = binary_img[ st[1]:st[1]+st[3], st[0]:st[0]+st[2]]            # Crop del objeto "i"
